...
public class PaintPanel extends View
{
    ...
    public float xPosistion_Double_tap; //to save x position before double tap
    public float yPosition_Double_tap; //to save y posistion before double tap
    ...
    private void initialize(Context context)
    {
        ...
        targetImage = context.getResources().getDrawable(R.drawable.cat);
    ...
    private class MyGestureListener extends GestureDetector.SimpleOnGestureListener
    {
        ...
        public boolean onDoubleTap(MotionEvent me){
            //on double tap do the following.
            //fetch current position of the pointer with me.getX() and me.getY()
            final float x = me.getX();
            final float y = me.getY();
            ...
            if (inside)
            {
              ...
              //if the double tap was inside the image, scale accordingly
                //if scalefactor is 1 -> magnify by 3f
                //if scalefactor is already 3 -> minimize it to 1f
                if (scaleFactor == 1f) {
                    scaleFactor = 3f;
                    float focusX = me.getX(); //get current position of x
                    float focusY = me.getY(); //get current position of y
                    float offsetX = focusX - xPosition; //get offset of x
                    float offsetY = focusY - yPosition;//get off set of y
                    xPosistion_Double_tap = xPosition;
                    yPosition_Double_tap = yPosition;
                    //save the current position of x and y so that we can return
                    //the location of the image when we minimize
                    xRatio = offsetX / imageIntrinsicWidth;
                    yRatio = offsetY / imageIntrinsicHeight;
                    //calculate the x,y ratio from the offsetX with the new image width and height
                    xPosition = offsetX - xRatio * imageIntrinsicWidth * scaleFactor;
                    yPosition = offsetY - yRatio * imageIntrinsicHeight * scaleFactor;
                    //set new position of x and y image
                } else {
                    scaleFactor = 1f;
                    xPosition = xPosistion_Double_tap;
                    yPosition = yPosition_Double_tap;
                    //return the x and y value to previous x,y (aka before zoom)
                }


                return true;
            }
            //if the double tap was not insde the image, consume the touch
            return true;
        }
